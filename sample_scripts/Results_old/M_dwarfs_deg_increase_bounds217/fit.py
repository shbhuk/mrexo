...
def fit_mr_relation(Mass, Mass_sigma, Radius, Radius_sigma, save_path,
    ...
    if Mass_min is None:
        Mass_min = np.log10(max(min(Mass - Mass_sigma), 0.01)) - 0.2
    if Mass_max is None:
        Mass_max = np.log10(max(Mass + Mass_sigma)) + 0.2
    if Radius_min is None:
        Radius_min = min(np.log10(min(Radius - Radius_sigma)), -0.3) - 0.2
    if Radius_max is None:
        Radius_max = np.log10(max(Radius + Radius_sigma)) + 0.2
