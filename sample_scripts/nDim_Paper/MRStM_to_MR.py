...
ConditionString = 'm,stm|r'
ConditionName = '3D_'+ConditionString.replace('|', '_').replace(',', '_')

ResultDirectory = r'C:\Users\skanodia\Documents\GitHub\mrexo\sample_scripts\TestRuns\AllPlanet_RpLt4_StMlt1.5_MRStM_d100_100MC_100BS'
PlotFolder = os.path.join(ResultDirectory, ConditionName)
PlotFolder = os.path.join(ResultDirectory, '3D_m_r_stm')
...
norm = matplotlib.colors.Normalize(vmin=StellarMassArray[0], vmax=StellarMassArray[-1])
cmap = matplotlib.cm.Spectral

CombinedQuery = np.rollaxis(np.array(np.meshgrid(StellarMassArray, PlanetRadiusArray)), 0, 3).reshape(len(StellarMassArray)*len(PlanetRadiusArray),2)
...
# plt.figure()
for i,stm in enumerate(StellarMassArray):
	if i%4 == 0: 	plt.axvline(10**MeanPDF[i], linestyle='dashed', color=cmap(norm(stm)), label=r"3D f(m|r, {:.2f}".format(stm) + r" $M_{\odot}$)")
	else: plt.axvline(10**MeanPDF[i], linestyle='dashed', color=cmap(norm(stm)))
...
plt.title(r"f(m|r=1.5 $R_{\oplus}$, stm)")
...
plt.savefig(os.path.join(r"C:\Users\skanodia\Documents\GitHub\Paper_ndim_mrexo\Plots", "MRStM_MR_Rp1.5_v2.pdf"))
