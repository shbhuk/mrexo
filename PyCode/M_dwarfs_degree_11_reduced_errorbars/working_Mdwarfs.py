...
M_sigma = (abs(t['pl_masseerr1']) + abs(t['pl_masseerr2']))/20
R_sigma = (abs(t['pl_radeerr1']) + abs(t['pl_radeerr2']))/20
...
if __name__ == '__main__':
    ...
    a = MLE_fit_bootstrap(Mass = M_obs, Radius = R_obs, Mass_sigma = M_sigma, Radius_sigma = R_sigma, Mass_max = Mass_max,
                        Mass_min = Mass_min, Radius_max = Radius_max, Radius_min = Radius_min, degree_max = 20, select_deg = 11, Log = True, num_boot = 50,
                        location = os.path.join(os.path.dirname(__file__),'M_dwarfs_degree_11_reduced_errorbars'), abs_tol = 1e-10)
