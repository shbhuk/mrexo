...
def calculate_conditional_distribution(ConditionString, DataDict, 
	...
	INPUTS:
		...
		MeasurementDict: Example:
			...
			MeasurementDict['r'] = [[np.log10(4)], [np.nan]]
			MeasurementDict['p'] = [[np.log10(0.9999)], [np.nan]]
	...
	ConditionPDF = UnivariateSpline(LHSSequence[0], ConditionalDist).integral(
	...
	MeanPDF = UnivariateSpline(LHSSequence[0], ConditionalDist*LHSSequence[0]).integral(
		DataDict["ndim_bounds"][LHSDimensions[0]][0], DataDict["ndim_bounds"][LHSDimensions[0]][1]) / ConditionPDF
		
	VariancePDF = UnivariateSpline(LHSSequence[0], ConditionalDist*(LHSSequence[0]**2)).integral(
		DataDict["ndim_bounds"][LHSDimensions[0]][0], DataDict["ndim_bounds"][LHSDimensions[0]][1]) / ConditionPDF - (MeanPDF**2)
	...
	return ConditionPDF, MeanPDF, VariancePDF
