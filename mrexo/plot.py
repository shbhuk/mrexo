...
def plot_m_given_r_relation(result_dir):
    ...
    handles = [mean_line, red_patch, blue_patch]
...
def plot_r_given_m_relation(result_dir):
    ...
    ax1.plot(M_points,R_cond_M,  color = 'darkblue', lw = 2) # Full dataset run
    ax1.fill_between(M_points,R_cond_M_lower,R_cond_M_upper,alpha = 0.3, color = 'r') # Full dataset run
    ax1.fill_between(M_points,lower_boot,upper_boot,alpha = 0.3, color = 'b') # Bootstrap result



    mean_line = Line2D([0], [0], color='darkblue', lw = 2,label = 'Mean of conditional distribution of R given M from full dataset run')
    red_patch = mpatches.Patch(color='r', alpha = 0.3,  label=r'16%-84% quantile of conditional distribution of R given M from full dataset run  ')
    blue_patch = mpatches.Patch(color='b', alpha = 0.3, label=r'16%-84% quantile of the MEAN of the conditional distribution of R given M from bootstrap')
    handles = [mean_line, red_patch, blue_patch]
