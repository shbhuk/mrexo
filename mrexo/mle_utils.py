...
def _find_indv_pdf(a, deg, deg_vec, a_max, a_min, x_std=None, abs_tol=1e-8, Log=True):
    '''
    ...
    if x_std == None:
        x_std = (a - a_min)/(a_max - a_min)
        x_beta_indv = np.array([_beta_pdf(x_std, a=d, b=deg - d + 1)/(a_max - a_min) for d in deg_vec])
    else:
        x_beta_indv = np.array([integrate_function(data=x, data_std=x_std, deg=deg, degree=d, a_max=a_max, a_min=a_min, abs_tol=abs_tol, Log=Log) for d in deg_vec])
    return x_beta_indv
...
def cond_density_quantile(y, y_max, y_min, x_max, x_min, deg, deg_vec, w_hat, y_std=None, qtl=[0.16,0.84], abs_tol=1e-8):
    ...
    y_beta_indv = _find_indv_pdf(x=y, deg=deg, deg_vec=deg_vec, a_max=y_max, x_min=y_min, x_std=y_std, abs_tol=abs_tol, Log=False)
